...
> = {
  hot: {
    label: "HOT",
    copy: "15분의 온기 — 근육의 긴장을 녹이고 스킨케어의 깊이를 더합니다. 따뜻한 세라믹 표면이 피부 온도를 높여 모공을 열고, 세럼과 크림의 유효 성분이 피부 깊숙이 흡수되도록 돕습니다.",
    details: [
      "전자레인지 30초로 간편하게 가열",
      "저녁 스킨케어 루틴의 마지막 단계",
      "혈액순환 촉진 & 근육 이완",
  ...
  cool: {
    label: "COOL",
    copy: "30분의 청량함 — 부기를 가라앉히고 무너진 라인을 정교하게 조각합니다. 차가운 세라믹이 림프 순환을 자극하여 밤사이 쌓인 수분과 노폐물을 빠르게 배출시키고, 처진 페이스라인을 선명하게 정돈합니다.",
    details: [
      "냉동실 1시간이면 준비 완료",
      "아침 기상 직후 세안 후 사용",
      "림프 순환 촉진으로 붓기 완화",
      ...
      <div className="relative max-w-screen-xl mx-auto px-6 md:px-8 py-20 md:py-32">
        ...
        <AnimatePresence mode="wait">
          <m.div
            ...
            exit={{ opacity: 0, y: -20 }}
            transition={{ duration: DURATION.standard, ease: SARAJOO_EASE }}
            className="grid grid-cols-1 md:grid-cols-[3fr_2fr] gap-8 md:gap-12"
          >
            {/* Image placeholder */}
            <div className="aspect-[4/3] md:aspect-auto relative overflow-hidden border border-brand-taupe/10">
              <Image src={imgSrc(activeTab === "hot" ? "/images/product/hot-stone.jpg" : "/images/ritual/cool-pool.jpg")} alt={activeTab === "hot" ? "HOT 괄사 사용법" : "COOL 괄사 사용법"} fill className="object-cover object-[center_65%]" sizes="(max-width: 768px) 100vw, 60vw" />
            </div>
